...
@section('content')
    ...
    <div style="width:95%;margin:auto">
       ...
       <a  onclick="del_row('<?= $user->id ?>','<?= url('admin/user') ?>','<?= Session::token() ?>')" class="btn btn-danger">حذف کاربر</a>
        ...
        <div class="info" style="font-size: 18px;border: black solid; direction: rtl; text-align: right; margin: auto;background-color: white;color: black;padding: 30px;margin-top: 10px">
            <span>نام: </span>&nbsp;
            {{ $name or null }}<br>
            ...
            {{ $phone  or null }}<br>
            <span>کد ملی: </span>&nbsp;
            {{ $person_id  or null }}<br>
            <span>کد معرف: </span>&nbsp;
            {{ $moaref_id  or null }}<br>
        </div>
